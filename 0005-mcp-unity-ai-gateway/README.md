...
> Artigo com implementação · Governança de ferramentas
...
O texto recuperado está em [artigo-linkedin.md](artigo-linkedin.md). A versão curta para revisão está em [post-linkedin.md](post-linkedin.md). A implementação isola o conceito **governança de ferramentas** usando um cenário JSON seguro, uma função Python, relatório e teste.
...
Governança de ferramentas, configuração como dados, função determinística, cenário mínimo reproduzível, teste automatizado, rastreabilidade e distinção entre PoC e produção.
