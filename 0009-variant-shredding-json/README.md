...
> Artigo com implementação · Projeção de JSON
...
O texto recuperado está em [artigo-linkedin.md](artigo-linkedin.md). A versão curta para revisão está em [post-linkedin.md](post-linkedin.md). A implementação isola o conceito **projeção de json** usando um cenário JSON seguro, uma função Python, relatório e teste.
...
cd "0009-variant-shredding-json"
```
...
Projeção de JSON, configuração como dados, função determinística, cenário mínimo reproduzível, teste automatizado, rastreabilidade e distinção entre PoC e produção.
