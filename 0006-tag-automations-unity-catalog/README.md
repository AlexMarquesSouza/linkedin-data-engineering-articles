...
> Artigo com implementação · Classificação automática
...
O texto recuperado está em [artigo-linkedin.md](artigo-linkedin.md). A versão curta para revisão está em [post-linkedin.md](post-linkedin.md). A implementação isola o conceito **classificação automática** usando um cenário JSON seguro, uma função Python, relatório e teste.
...
cd "0006-tag-automations-unity-catalog"
```
...
Classificação automática, configuração como dados, função determinística, cenário mínimo reproduzível, teste automatizado, rastreabilidade e distinção entre PoC e produção.
