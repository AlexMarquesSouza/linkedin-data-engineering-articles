...
> Artigo com implementação · Aplicação de eventos parciais
...
O texto recuperado está em [artigo-linkedin.md](artigo-linkedin.md). A versão curta para revisão está em [post-linkedin.md](post-linkedin.md). A implementação isola o conceito **aplicação de eventos parciais** usando um cenário JSON seguro, uma função Python, relatório e teste.
...
cd "0011-auto-cdc-partial-bitemporal"
```
...
Aplicação de eventos parciais, configuração como dados, função determinística, cenário mínimo reproduzível, teste automatizado, rastreabilidade e distinção entre PoC e produção.
