...
> Artigo com implementação · Compatibilidade de clientes
...
O texto recuperado está em [artigo-linkedin.md](artigo-linkedin.md). A versão curta para revisão está em [post-linkedin.md](post-linkedin.md). A implementação isola o conceito **compatibilidade de clientes** usando um cenário JSON seguro, uma função Python, relatório e teste.
...
cd "0017-iceberg-managed-sharing"
```
...
Compatibilidade de clientes, configuração como dados, função determinística, cenário mínimo reproduzível, teste automatizado, rastreabilidade e distinção entre PoC e produção.
