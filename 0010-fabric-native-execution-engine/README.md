...
> Artigo com implementação · Fallback de execução
...
O texto recuperado está em [artigo-linkedin.md](artigo-linkedin.md). A versão curta para revisão está em [post-linkedin.md](post-linkedin.md). A implementação isola o conceito **fallback de execução** usando um cenário JSON seguro, uma função Python, relatório e teste.
...
cd "0010-fabric-native-execution-engine"
```
...
Fallback de execução, configuração como dados, função determinística, cenário mínimo reproduzível, teste automatizado, rastreabilidade e distinção entre PoC e produção.
