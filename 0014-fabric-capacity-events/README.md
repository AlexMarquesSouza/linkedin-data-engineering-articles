...
> Artigo com implementação · Alertas antes do throttling
...
O texto recuperado está em [artigo-linkedin.md](artigo-linkedin.md). A versão curta para revisão está em [post-linkedin.md](post-linkedin.md). A implementação isola o conceito **alertas antes do throttling** usando um cenário JSON seguro, uma função Python, relatório e teste.
...
cd "0014-fabric-capacity-events"
```
...
Alertas antes do throttling, configuração como dados, função determinística, cenário mínimo reproduzível, teste automatizado, rastreabilidade e distinção entre PoC e produção.
