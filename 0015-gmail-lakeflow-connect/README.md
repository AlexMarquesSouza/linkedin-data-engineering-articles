...
> Artigo com implementação · Cursor e deduplicação
...
O texto recuperado está em [artigo-linkedin.md](artigo-linkedin.md). A versão curta para revisão está em [post-linkedin.md](post-linkedin.md). A implementação isola o conceito **cursor e deduplicação** usando um cenário JSON seguro, uma função Python, relatório e teste.
...
cd "0015-gmail-lakeflow-connect"
```
...
Cursor e deduplicação, configuração como dados, função determinística, cenário mínimo reproduzível, teste automatizado, rastreabilidade e distinção entre PoC e produção.
