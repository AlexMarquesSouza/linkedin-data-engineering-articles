...
> Artigo com implementação · Recuperação lexical local
...
O texto recuperado está em [artigo-linkedin.md](artigo-linkedin.md). A versão curta para revisão está em [post-linkedin.md](post-linkedin.md). A implementação isola o conceito **recuperação lexical local** usando um cenário JSON seguro, uma função Python, relatório e teste.
...
Recuperação lexical local, configuração como dados, função determinística, cenário mínimo reproduzível, teste automatizado, rastreabilidade e distinção entre PoC e produção.
